$PBExportHeader$w_data_explorer170310.srw
$PBExportComments$Data Explorer (TreeView and ListView)
forward
global type w_data_explorer170310 from w_base_case
end type
type cb_3 from commandbutton within w_data_explorer170310
end type
type cb_1 from commandbutton within w_data_explorer170310
end type
type lv_1 from listview within w_data_explorer170310
end type
type tv_1 from treeview within w_data_explorer170310
end type
type st_vertical from statictext within w_data_explorer170310
end type
end forward
...
global w_data_explorer170310 w_data_explorer170310
...
// 检查treeview数据是否正确
//is_resultlog += wf_result_add("testcasepoint1 of_set_tv_item", "Label:" + atvi_New.Label +";Data:" + string(atvi_New.Data), True )
is_tree_data += "Label:" + atvi_New.Label +";Data:" + string(atvi_New.Data) + ";"
...
ll_count =  ids_Data[li_Level].Retrieve(li_Parm, li_RepID)
//检查检索到的数据是否正确
//is_resultlog += wf_result_add("testcasepoint1 of_retrieve_data", "datastore rows:" + string(ll_count) +";parm:" + string(li_Parm) +";RepID" + string(li_RepID), True)
is_tree_data += "datastore rows:" + string(ll_count) +";parm:" + string(li_Parm) +";RepID" + string(li_RepID) + ";"
Return ll_count
...
For li_Cnt = 1 To ai_Rows
	...
	If tv_1.InsertItemLast(al_Parent, ltvi_New) < 1 Then
		...
		is_resultlog += wf_result_add("testcasepoint1 of_add_tv_items",  "Error inserting item", False)
		Return -1
	End If
Next
...
//		MessageBox("Error", "Error adding item to the ListView", Exclamation!)
		is_resultlog += wf_result_add("testcasepoint1 of_add_lv_items", "Error adding item to the ListView", False)
		Return -1
	End If
Next
...
Choose Case ai_Level
	...
	Case 4
		alvi_New.Label = String(ids_Data[3].Object.id[ai_Row]) + "~t" + &
									String(ids_Data[3].Object.order_date[ai_Row], 'mmm dd, yyyy, h "hrs and" m "min"') + "~t" + &
	...
	Case 5
		alvi_New.Label = ids_Data[4].Object.product_description[ai_Row] + "~t" + &
									...
									String(ids_Data[4].Object.ship_date[ai_Row], 'mmm dd, yyyy, h "hrs and" m "min"') + "~t" + &
...
// 检查listview数据是否正确
//is_resultlog += wf_result_add("testcasepoint1 of_set_lv_item", "Label:" + alvi_New.Label +";Data:" + string(alvi_New.Data), True )
...
public subroutine of_testcase_point_module ();Long				ll_Root, ll_hand
TreeViewItem	ltvi_Root
string ls_data_test, ls_filename
...
// 再选择树下层结点一次
//tv_1.ChildTreeItem!
ll_hand = tv_1.FindItem(ChildTreeItem!, ll_Root)
do while ll_hand > 0
	tv_1.ExpandItem(ll_hand)
	tv_1.SelectItem(ll_hand)
	ll_hand = tv_1.FindItem(ChildTreeItem!, ll_hand)	
loop

n_resource_manager ln_resource_manager
ls_filename = 'w_data_explorer170310' +"_tree_data"
ls_data_test = ln_resource_manager.f_get_resource(ls_filename+ ".txt")

is_resultlog += wf_result_add("testcasepoint1", "treeview and listview load data", ls_data_test = is_tree_data )
end subroutine

on w_data_explorer170310.create
int iCurrent
call super::create
...
on w_data_explorer170310.destroy
call super::destroy
destroy(this.cb_3)
destroy(this.cb_1)
destroy(this.lv_1)
...
event resize;call super::resize;// Do not run the first time the window is opened.
...
ids_Data[1].DataObject = "d_sales_reps170310"

ids_Data[2] = Create DataStore
ids_Data[2].DataObject = "d_sales_rep_customers170310"

ids_Data[3] = Create DataStore
ids_Data[3].DataObject = "d_sales_rep_customer_orders170310"

ids_Data[4] = Create DataStore
ids_Data[4].DataObject = "d_salesorder_items170310"
...
string largepicturename[] = {"Globals!","AddWatch!","Library!","DosEdit!"}
integer largepicturewidth = 32
integer largepictureheight = 32
long largepicturemaskcolor = 12632256
string smallpicturename[] = {"Globals!","AddWatch!","Library!","DosEdit!"}
...
string picturename[] = {"Globals!","AddWatch!","Library!","DosEdit!","Custom050!"}
